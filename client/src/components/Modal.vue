          ...
          <div class="field">
            <div class="control">
              <button
                @click.prevent="
                  ($event) => {
                    updateLocalTodo($event, inputText);
                    closeModal();
                    inputText = '';
                    if (isLoggedIn) {
              ...
              <span class="has-text-danger">Error {{ errMsg }}</span>
            </div>
          </div>
        </form>
      </div>
...
export default defineComponent({
  ...
  watch: {
    editResponse: function (newValue: EditTodoResponse) {},
  },
  methods: {
    ...
    updateLocalTodo(event: Event, input: string): void {
      console.log("submit edit todo button inside the edit todo modal", event);
      store.commit(
        "todos/EDIT_TODO" as RootCommitType,
        {
          text: input,
          id: this.modalContext?.todoId,
        },
        { root: true }
      );
    },
